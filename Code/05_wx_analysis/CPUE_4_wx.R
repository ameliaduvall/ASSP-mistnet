...
## use RDS file
CPUE <- readRDS(here("Working", "cpue.RDS")) # 237 sessions
...
## include flagged sessions this time
...
final <- CPUE %>% 
  ...
  dplyr::select(session_ID, island_code, site_code, lat, long, session_year, session_date, start, end, flagged)

## Export
write.csv(final, here("Working", "ASSP_4_wx_v2.csv"))
saveRDS(final, here("Working", "ASSP_4_wx_v2.RDS"))
