...
## Filter data to ASSP species and remove SNRs
ASSP <- banding %>% 
...
month_BP <- ggplot(metadata, aes(as.character(session_month), BPfreq_Y)) +
  geom_boxplot() +
  stat_n_text(y.pos = 0.3) +
  xlab("Month") + ylab("Assumed Breeder Frequency") +
  theme_bw()
month_BP
## need to fix text position
```
...
# p1a <- ggplot(ASSP) + geom_density(aes(x = mass_corr, fill = island_code), alpha = 0.2) +
#   xlab("Culmen") +
#   theme_cowplot(12)
# p1a
# 
# p1b <- ggplot(ASSP) + geom_density(aes(x = mass_corr)) + facet_wrap(~island_code) +  
#   geom_vline(aes(xintercept = mean(mass_corr, na.rm = TRUE)),
...
## RECAPTURES
```{r recap}
recap <- group_by(.data = ASSP, session_year, island_code) %>%
...
summary(ASSP$BP)
unique(ASSP$BP) 
BPfreq_Y_sum <- metadata$BPfreq_Y
