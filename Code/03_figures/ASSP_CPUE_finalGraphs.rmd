...
# setwd("~/Users/ekelseyold/Documents/git/ASSP-mistnet/Data/cleaned_data")
...
### READ IN DATA
banding <- readRDS(here("Data", "cleaned_data", "captures.RDS"))
metadata <- readRDS(here("Data", "cleaned_data", "cpue.RDS"))
...
## Filter data to ASSP species and remove same night recaptures (SNRs) and unbanded individuals 
ASSP <- banding %>% 
  filter(species == "ASSP" & recapture != "SNR" & band_no != "notbanded")  # 3799 observations
  
ASSP_BP <- ASSP %>% 
  filter(BP != "NR" & BP != "NA") %>% 
  mutate(assumeBreed = mosaic::derivedFactor( # assumed breeding is based on broodpatch score, which was recorded in multiple ways over the years
...
year <- ggplot(metadata, aes(as.character(session_year), CPUEstd)) +
  ...
  ggtitle("Channel Islands National Park") +
  theme_cowplot(12) # theme_bw()
...
ggsave(here("Figures", "cpue_by_year.png"))
...
## 2021 sites only
sites_only <- ggplot(metadata, 
                  aes(as.character(session_year), CPUEstd, color = site_code)) +
  geom_boxplot(position = position_dodge2(preserve = "single")) +
  facet_wrap(~site_code) + xlab("Year") + ylab("Standardized Catch-Per-Unit-Effort") +
  ggtitle("2021 Mistnetting Sites") +
  stat_n_text(size = 3.5, y.pos = 0.41) +
  theme_cowplot(12) +
  theme(axis.text.x = element_text(angle = 90),
        panel.spacing = unit(2, "lines"),
        legend.position = "none")
sites_only %+% subset(metadata, session_year %in% c("1994", "1995", "1996", "2004", "2005", "2006", "2007", "2009", 
                                            "2010", "2011", "2014", "2015", "2016", "2017", "2018", "2021") &
                     site_code == "AP" | site_code == "ESP" | site_code == "SR")
#ggsave(width = 7, height = 5, dpi=300, filename = "~/WERC-SC/ASSP_share/figures/CPUE_yr.pdf")
ggsave(width = 15, height = 5, here("Figures", "sites_2021_cpue_by_year.png"))

SBI_SCI_year <- ggplot(metadata, 
                  aes(as.character(session_year), CPUEstd, color = island_code)) +
  geom_boxplot() +
  facet_wrap(~island_code) +
  xlab("Year") + ylab("Standardized Catch-Per-Unit-Effort") +
  ggtitle("Santa Barbara and Santa Cruz Islands") +
  stat_n_text(size = 3.5, y.pos = 0.41) +
  theme_cowplot(12) +
  theme(axis.text.x = element_text(angle = 90),
        legend.position = "none")
SBI_SCI_year %+% subset(metadata, session_year %in% c("1994", "1995", "1996", "2004", "2005", "2006", "2007", "2009", 
                                            "2010", "2011", "2014", "2015", "2016", "2017", "2018", "2021") &
                     island_code == "SBI" | island_code == "SCI")
#ggsave(width = 7, height = 5, dpi=300, filename = "~/WERC-SC/ASSP_share/figures/CPUE_yr.pdf")
ggsave(width = 12, height = 5, here("Figures", "SBI_SCI_cpue_by_year.png"))
```
...
ggsave(here("Figures", "cpue_month_year.png"))
```
...
metadata_ARU <- metadata %>% 
  filter(site_code %in% c("AP", "ESP", "SR", "PI")) %>%
  filter(session_year %in% c("2016", "2017", "2018", "2021"))

isl_yr <- ggplot(metadata_ARU, aes(as.character(session_year), CPUEstd, color = site_code)) +
  geom_boxplot() +
  facet_grid(.~site_code) +
  xlab("Sites") + ylab("Standardized CPUE") +
  stat_n_text(size = 3, y.pos = 0.3) +
  theme_cowplot(12) +
  theme(axis.text.x = element_text(angle = 90),
        panel.spacing = unit(2, "lines"),
        legend.position = "none") +
  geom_hline(aes(yintercept = mean(CPUEstd, na.rm = TRUE)),
             colour = "black", linetype ="longdash", size = .5)
isl_yr
#isl_yr %+% subset(metadata_ARU, session_year %in% c("2016", "2017", "2018", "2021"))
#ggsave(width = 7, height = 5, dpi=300, filename = "~/WERC-SC/ASSP_share/figures/CPUE_ARUsites.pdf")
ggsave(width = 8, height = 5, here("Figures", "cpue_ARU_sites.png"))
```
...
## CATCH TIMES
```{r catch times 1}
# couldn't figure out how to calcuLate catch time past sunset in R. Exported to excel and subtracted catch time from sunset time, then imported back:
