...
### READ IN DATA
banding <- readRDS(here("Working", "captures.RDS"))
...
ggplot(metadata, aes(session_year, island_code, color = org)) +  
  ...
  scale_x_continuous(name="Years of Mistnetting effort", breaks=seq(1994, 2018, 4)) +    ylab("Island") +  scale_color_brewer(palette="BrBG") +  
  theme_cowplot(12)
...
band.recaps <- unique(recap2$band_no)

recap3 <- ASSP %>%
  filter(band_no %in% c(band.recaps)) %>%
  arrange(band_no) %>%
  dplyr::select(band_no, recapture, session_year, session_date, island_code, site_code)

recap3.sum <- recap3 %>%
  group_by(band_no) %>%
  summarize(n = n())

# write.csv(recap3, here("Output", "recap_by_band.csv"), row.names = FALSE)

recap.no <- recap3 %>%
  filter(recapture == "N") %>%
  mutate(capture_year = session_year, capture_date = session_date,
         capture_island = island_code, capture_site = site_code) %>%
  dplyr::select(-c(recapture))

recap.yes <- recap3 %>%
  filter(recapture == "Y") %>%
  mutate(recapture_date = session_date,
         recapture_year = session_year,
         recapture_island = island_code,
         recapture_site = site_code) %>%
  dplyr::select(band_no, recapture_year, recapture_date,
                recapture_island, recapture_site)

recap.table <- left_join(recap.yes, recap.no, by = "band_no") %>%
  dplyr::select(band_no, capture_year, capture_date, capture_island, capture_site, 
                recapture_year, recapture_date, recapture_island, recapture_site) %>%
  arrange(capture_year, recapture_date) %>%
  rename("Band Number" = band_no,
         "Capture Year" = capture_year,
         "Capture Date" = capture_date,
         "Capture Island" = capture_island,
         "Capture Site" = capture_site,
         "Recapture Year" = recapture_year,
         "Recapture Date" = recapture_date,
         "Recapture Island" = recapture_island,
         "Recapture Site" = recapture_site) 

write.csv(recap.table, here("Output", "recap_table.csv"), row.names = FALSE)
```
