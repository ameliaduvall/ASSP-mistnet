...
Notes
We decided to use three random effects (site code, year, month) to account for some variation in CPUE. 

* see Russ Bradley et al. 2011 paper for ideas
* what are our questions?
* include other env'l covariates? index of productivity
* how to include brood   patch
* include year^2, month^2, etc?
* do we need to check the CPUE file..? or wx data file? can't remember if that needs re-run (remove sites w. NA)
* include session night as a covariate?
* wind speed at midnight
* change random effects? include month as random effect? and series_ID? too many RE?
* why is CPUE changing temporally and spatially?
* can do same thing with frequency of assumed breeders CPUE? 
* and does this inform how we approach future mistnetting efforts? 
* maybe month should be a predictor? 
* collapse down into island (remove site) 
...
## moonT * Illu
## using hourly data was sum of Moonup
## ek to send new RDS file with 4 new variables--moonT, moon index, wind speed at midnight, avg wind speed
## multity 2 moon variables
...
nmRE <- lmer(logCPUE ~ 1 + (1 | site_code) + (1 | session_year) + (1 | session_month), data = df)
sumary(nmRE)
plot(nmRE)
...
abline(h=0, lty = "dashed") # also does not look too bad, some heteroscedasticity 

## try sq rt of absolute value of residuals v fitted values
sqrtresid <- sqrt(abs(fitted(nmRE)))

plot(fitted(nmRE), sqrtresid, las = 1, pch = 16,
xlab = "Fitted", ylab = "Residuals",
main = "Residuals vs fitted")
abline(h=0, lty = "dashed")

## Levene's test
resids <- residuals(nmRE)
g1 <- resids[resids < median(resids)]
g2 <- resids[resids < median(resids)]
var.test(g1, g2)
...
* illu = 'illuminatedFraction' = fraction of moon’s visible disk that is illuminated
