...
## scale predictors
df <-  dfr %>% 
  mutate(tot_precip = scale(tot_precip),wind_u = scale(wind_u),
         wind_v = scale(wind_v), wave_height = scale(wave_height),
         wave_dir = scale(wave_dir), wave_period = scale(wave_period),
         tot_cloud = scale(tot_cloud), low_cloud = scale(low_cloud),
         pressure = scale(pressure), surface_pressure = scale(surface_pressure),
         sst = scale(sst), temp_c = scale(temp_c), dew_point = scale(dew_point),
         Moonup = scale(Moonup), MoonAltitude = scale(MoonAltitude), 
         Illu = scale(Illu))
