...
sessions <- readRDS(here::here("Data", "database", "sessions.RDS"))
captures <- readRDS(here::here("Data", "database", "captures.RDS"))
...
# Filter to ASSP spp only and remove SNRs and unbanded individuals
ASSP <- captures %>%
...
#   mutate(min_1 = as.numeric(net_close_1 - net_open_1),
#          min_2 = as.numeric(net_close_2 - net_open_2),
#          min_3 = as.numeric(net_close_3 - net_open_3),
#          min_4 = as.numeric(net_close_4 - net_open_4),
#          min_5 = as.numeric(net_close_5 - net_open_5)) %>% 
...
#          min_1_std = as.numeric(net_close_1_std - net_open_1),
#          min_2_std = as.numeric(net_close_2_std - net_open_2),
#          min_3_std = as.numeric(net_close_3_std - net_open_3),
#          min_4_std = as.numeric(net_close_4_std - net_open_4),
#          min_5_std = as.numeric(net_close_5_std - net_open_5)) %>% 
...
#          min_std = as.double(if_else(min_std == 0, "NA", as.character(min_std))),# new line
...
mins <- function(x) lubridate::time_length(x, unit = "minutes")  # helper

cpue_v1 <- getSunlightTimes(data = sun_vec, keep = "sunset", tz = "America/Los_Angeles") %>%
  mutate(
    std_ending = sunset + hours(5) + minutes(18),    # 5.3 hours after sunset
    app_sunset = sunset
  ) %>%
  select(-sunset) %>%  # keep only app_sunset
  left_join(sun_vec, by = c("date", "lat", "lon")) %>%
  left_join(sessions, by = c("session_ID", "site_code")) %>%
  # raw minutes open (explicitly in minutes)
  mutate(
    min_1 = mins(net_close_1 - net_open_1),
  ...
  # cap closes at std_ending 
  mutate(
    net_close_1_std = pmin(net_close_1, std_ending),
  ...
  # minutes before standard ending
  mutate(
    min_1_std = mins(net_close_1_std - net_open_1),
  ...
  # clamp negatives to 0 and fill NAs with 0, numerically
...
saveRDS(cpue, here("Data", "database", "cpue.RDS"))
write.csv(cpue, here("Data", "database", "cpue.csv"), row.names = FALSE)
```