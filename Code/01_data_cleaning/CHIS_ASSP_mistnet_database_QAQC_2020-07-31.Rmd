...
```{r readin}
captures <- read.csv(here("Data", "captures.csv"), na.strings = c("NA", "ND"))
sessions <- read.csv(here("Data", "sessions.csv"), na.string = c("NA", "ND"))
# cpue <- read.csv(here("Data", "cpue.csv"), na.strings = c("NA", "ND"))

# Fix date/times
captures <- captures %>%
  mutate_at((c("capture_time", "release_time")), ~ as.POSIXct(.x, format="%m/%d/%Y %H:%M"))

sessions <- sessions %>%
  mutate_at((c("net_open_1", "net_open_2", "net_open_3", "net_open_4", "net_open_5",
               "net_close_1", "net_close_2", "net_close_3", "net_close_4", "net_close_5" )), ~ as.POSIXct(.x, format="%m/%d/%Y %H:%M"))
```
...
# Boxplot of values
ggplot(ASSP, aes(x = as.factor(year), y = mass_corr, fill = island_code)) +
         geom_boxplot() +
          theme_bw()
...
# Boxplot of values
ggplot(ASSP, aes(x = as.factor(year), y = culmen, fill = island_code)) +
         geom_boxplot() +
          theme_bw()
...
# Boxplot of values
ggplot(ASSP, aes(x = as.factor(year), y = skull_length, fill = island_code)) +
         geom_boxplot() +
          theme_bw()
...
# Boxplot of values
ggplot(ASSP, aes(x = as.factor(year), y = tarsus, fill = island_code)) +
         geom_boxplot() +
          theme_bw()
...
# Boxplot of values
ggplot(ASSP, aes(x = as.factor(year), y = tail, fill = island_code)) +
        geom_boxplot() +
        theme_bw()
...
## Session ID
Cross-reference session_ID in Banding sheet with session_ID in CPUE sheet to confirm that all mistnetting sessions are listed in CPUE sheet. 
```{r sessionID}
# Anti join returns records in ASSP df are not matched by "session_ID" in cpue df
ASSPorg <- anti_join(ASSP, cpue, by = "session_ID")
missing.ID <- unique(ASSPorg$session_ID)
write.csv(missing.ID, here("Data", "missingID.csv"))
ASSP.missing.ID <- ASSP %>%
  filter(session_ID == "19940514_ANI_RR" |
           session_ID == "20040819_SCI_ND" |
           session_ID == "20040624_SCI_ND" |
           session_ID == "20150715_SMI_PI1" |
           session_ID == "20160802_SCI_SR1" |
           session_ID == "20160612_SBI_AP" |
           session_ID == "20170612_SBI_AP" |
           session_ID == "20100606_SBI_SRO" |
           session_ID == "20120827_SBI_ESP" |
           session_ID == "20110727_ANI_RR" |
           session_ID == "20060601_SCI_SR1")
```
